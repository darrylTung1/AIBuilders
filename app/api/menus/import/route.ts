...
export async function POST(request: Request) {
  try {
    ...
    if (contentType.includes("application/json")) {
      const body = await request.json() as { sql: string; menuName?: string };
      sql = body.sql;
      ...
      return NextResponse.json({ error: "Send JSON with sql, menuName or multipart with file, menuName" }, { status: 400 });
    }
    ...
    const [menu] = await db.insert(menus).values({ name: menuName }).returning();
